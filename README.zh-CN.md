...
- **Agents** — 从内置 **HR 助手样例**、空白模板或上传的 `.py` 文件创建
  agent;浏览器内 CodeMirror 编辑代码;追加 pip 依赖;**部署**会构建代码包
...
- **数据集** — 评估数据集为 `{prompt, context?}` 条目,`context` 是发送时
  ...
  表格内编辑,或上传 JSON / JSONL。notebook 的 10 条基线提示词作为样例内置。
...
- **洞察** — 对 agent 会话做触诊分析(AgentCore Insights,公开预览):
  ...
  **"用洞察触诊"**)或最近时间窗口。洞察复用批量评估 API —— 传
  ...
  评估)。内置一套**故障注入提示词**样例数据集(不存在的员工 ID、越权请求、
  诱导幻觉的问题),让失败模式可复现。报告历史服务端持久化。
